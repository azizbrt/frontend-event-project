...
const Navbar = ({ handleOrderPopup }) => {
  ...
  const handleLogout = () => {
    ...
    navigate("/"); // Go to the home page after logout
  };

  return (
    <div className="shadow-md bg-white dark:bg-gray-900 dark:text-white duration-200 relative z-40">
      {/* Top Section with Logo */}
      <div className="bg-gradient-to-r  bg-orange-500">
              ...
              {isAuthenticated ? (
                user?.isVerified ? (
                  <div className="relative">
                    ...
                    {profileMenuOpen && (
                      <div className="absolute right-0 mt-2 w-48 bg-white border rounded-lg shadow-lg z-10">
                        ...
                        {user.role === "admin" && (
                          <>
                            <Link to="/admin" className="block px-4 py-2 text-gray-700 hover:bg-gray-200">Tableau Admin</Link>
                            <Link to="/gestionnaire" className="block px-4 py-2 text-gray-700 hover:bg-gray-200">Tableau Gestionnaire</Link>
                          </>
                        )}
                        ...
                        {/* Profile and Logout */}
                        <Link to="/UpdateProfil" className="block px-4 py-2 text-gray-700 hover:bg-gray-200">Mon Profil</Link>
                        <button onClick={handleLogout} className="block w-full text-left px-4 py-2 text-gray-700 hover:bg-gray-200">Déconnexion</button>
                      </div>
                    )}
                  </div>
                ) : (
                  ...
                  <div className="bg-yellow-100 border border-yellow-400 text-yellow-800 px-4 py-3 rounded-md shadow-md max-w-md mx-auto mt-4">
                    ...
                    <Link to="/verify-email" className="inline-block mt-3 text-sm text-blue-600 hover:underline">➤ Aller à la page de vérification</Link>
                  </div>
                )
              ) : (
                // Not logged in
                <button onClick={handleLogin} className="bg-white text-orange-600 px-4 py-2 rounded-full flex items-center gap-2 shadow-md hover:bg-orange-100 transition-all">
                  <CgProfile className="text-xl" />
