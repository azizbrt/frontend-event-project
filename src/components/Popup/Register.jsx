...
const Register = ({ setRegisterPopup, setOrderPopup }) => {
  ...
  const handleSignUp = async (e) => {
    ...
    try {
      ...
      navigate("/verify-email", { state: { email } }); // Correction ici : user.email -> email
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 backdrop-blur-sm flex items-center justify-center">
      <div className="flex w-[700px] max-w-full bg-white dark:bg-gray-900 rounded-2xl shadow-lg relative overflow-hidden">
        {/* Close Button */}
        <button
          onClick={closePopup}
        ...
        {/* Left Image Section */}
        <div className="hidden md:flex items-center justify-center bg-white dark:bg-gray-800 w-1/2 p-10">
        <img src={registerImage} alt="Registration" className="max-h-60 object-contain" />
        </div>
